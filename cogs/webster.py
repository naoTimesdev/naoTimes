...
from nthelper.utils import DiscordPaginator, italic
from nthelper.webster import (
    ParserReplacer,
    WebsterDefinedWord,
    WebsterTokenParser,
...
def parse_md_xref_link(matched: T.Match[T.AnyStr]) -> T.AnyStr:
    ...
    return f"[{text_data}]({_DF_URL}{quote(url_id)})"


class WebsterCogs(commands.Cog):
    def __init__(self, bot: naoTimesBot) -> None:
        self.bot = bot
        self._mw = bot.merriam
        self.logger = logging.getLogger("cogs.webster.WebsterCogs")
        self._col = discord.Colour.from_rgb(48, 95, 122)
        # This is token formatter for Discord embed
        self._token_formatter: ParserReplacer = {
            "fmt-bold": r"**\g<content>**",
            "fmt-italic": r"*\g<content>*",
            "word-phrase": r"***\g<content>***",
            "word-quote": r"*\g<content>*",
            "word-headword-text": r"*\g<content>*",
    ...
    async def word_define_cmd(self, ctx: commands.Context, *, word: str):
        ...
        main_gen.checker()
        main_gen.set_generator(ftp_gen)
        self.logger.info("Sending results to user...")
        await main_gen.start(main_results, 25.0, None)
    ...
    @cog_ext.cog_slash(
        ...
        options=[
            manage_commands.create_option(
                name="word", description="Kata yang ingin dicari", option_type=3, required=True
            )
        ],
    )
    async def word_define_slash_cmd(self, ctx: SlashContext, word: str):
        if not word:
    ...
    async def thesaurus_cmd(self, ctx: commands.Context, *, word: str):
        ...
        main_gen = DiscordPaginator(self.bot, ctx)
        main_gen.checker()
        main_gen.set_generator(_generate_embed)
        self.logger.info("Sending results to user...")
        await main_gen.start(main_results, 25.0, None)


def setup(bot: naoTimesBot):
    bot.add_cog(WebsterCogs(bot))
