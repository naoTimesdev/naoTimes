...
from naotimes.music.lyrics import naoTimesLyricsResult
from naotimes.music.lyrics.client import LyricSource
...
NETEASE_ICON = "https://p.ihateani.me/kdrbldgu.png"
SPOTIFY_ICON = "https://www.scdn.co/i/_global/touch-icon-144.png"
...
def escape_html(html_entities: str) -> str:
    return html_unescape(html_entities)
...
class MusikPlayerCommand(commands.Cog):
    ...
    async def select_track(self, ctx: naoTimesContext, all_tracks: List[wavelink.Track]):
        ...
        messages = []
        messages.append("**Mohon pilih angka yang ingin anda tambahkan ke Bot!**")
        ...
        track_selections: List[Selection] = []
        for ix, track in enumerate(max_tracks, 1):
            track_selections.append(Selection(str(ix), f"ntplayer-{track.id}", track.title))

        selected = await ctx.select_single_button("\n".join(messages), track_selections, timeout=30.0)
        if selected is None:
            self.logger.warning("Track selection timed out, cancelling...")
            return MISSING
        if selected is False:
            self.logger.warning("Track selection cancelled, cancelling...")
            return None

        find_track: List[wavelink.Track] = list(
            filter(lambda x: f"ntplayer-{x.id}" == selected.name, all_tracks)
        )
        if not find_track:
            self.logger.warning("Track selection missing?, cancelling...")
            return MISSING
    ...
    async def musik_player_volume(self, ctx: naoTimesContext, volume: int = None):
        ...
        await ctx.send("🔊 Volume untuk sementara tidak bisa diubah karena masalah internal pemutar musik!")
        # XXX: Restore when I manage to fix lavaplayer stupid shit
        # Wrap from 0 to 1000
    ...
    async def musik_player_info(self, ctx: naoTimesContext):
        ...
        description.append(f"📍 **Node**: `{vc.node.identifier}`")
        spoti_mode = "Tidak"
    ...
    async def _spotify_lyrics_version(self, track: SpotifyDirectTrack, node: Node):
        ...
        for number, lyrics_part in enumerate(joined_lyrics, 1):
            embed = discord.Embed(colour=discord.Color.from_rgb(251, 48, 62))
    ...
    async def musik_player_lyric(self, ctx: naoTimesContext, *, override_query: str = None):
        ...
        self.logger.info(f"TrackLyric<{vc.guild}>: Searching for track <{current.title}>")
        matching_lyrics, error = await self.bot.ntlyrics.find_lyrics(current, override_query)
        if not matching_lyrics:
            self.logger.warning(f"TrackLyric<{vc.guild}>: No match for <{current.title}>")
            return await ctx.send(error.to_string())

        selected: Optional[naoTimesLyricsResult] = None
        if len(matching_lyrics) > 1:
            embed = discord.Embed(colour=discord.Color.from_rgb(227, 150, 64))
            ...
            selected = matching_lyrics[0]

        if selected is None:
            return await ctx.send("Dibatalkan!")

        song_title = selected.title
        self.logger.info(f"TrackLyric<{vc.guild}>: Requesting lyric <{current.title}>: {song_title}")
        await message_temp.edit(content=f"Mengambil lirik: {song_title}", embed=None)
        lyrics = await self.bot.ntlyrics.lookup_lyrics(selected.lyrics)
        ...
        embed_sets: List[discord.Embed] = []
        select_icon = MXM_ICON
        select_source = "Musixmatch"
        if selected.source == LyricSource.GENIUS:
            select_icon = GENIUS_ICON
            select_source = "Genius"
        elif selected.source == LyricSource.MUSIXMATCH:
            select_icon = MXM_ICON
            select_source = "Musixmatch"
        elif selected.source == LyricSource.SPOTIFY:
            select_icon = SPOTIFY_ICON
            select_source = "Spotify + Musixmatch"
        elif selected.source == LyricSource.NETEASE:
            select_icon = NETEASE_ICON
            select_source = "NetEase"
        for number, lyrics_part in enumerate(joined_lyrics, 1):
            embed = discord.Embed(colour=discord.Color.from_rgb(227, 150, 64))
            add_numbering = f" ({number}/{len(joined_lyrics)})"
            if len(joined_lyrics) < 2:
            ...
            embed.description = lyrics_part
            embed.set_footer(text=f"Lirik diambil dari {select_source}", icon_url=select_icon)
            embed_sets.append(embed)
        ...
        try:
            ...
            pass
        self.logger.info(f"GeniusLyric<{vc.guild}>: Sending lyric <{current.title}>: {song_title}")
    ...
    @musik_player.command(name="jeda", aliases=["pause"])
    @user_in_vc()
    @is_in_voice()
    @check_requirements(is_god, is_host)
    async def musik_player_pause(self, ctx: naoTimesContext):
        vc: wavelink.Player = ctx.voice_client
        if vc.is_paused():
            return await ctx.send("Penyetelan lagu sudah dijeda!", reference=ctx.message)
        if not vc.is_playing():
            return await ctx.send("Tidak ada lagu yang diputar!", reference=ctx.message)

        await vc.pause()
        prefix = self.bot.prefixes(ctx)
        await ctx.send(f"Lagu dijeda!\nPutar kembali dengan: `{prefix}m lanjut`", reference=ctx.message)

    @musik_player.command(name="lanjut", aliases=["resume", "lanjutkan"])
    @user_in_vc()
    @is_in_voice()
    @check_requirements(is_god, is_host)
    async def musik_player_resume(self, ctx: naoTimesContext):
        vc: wavelink.Player = ctx.voice_client
        if not vc.is_paused():
            return await ctx.send("Penyetelan sama sekali tidak dijeda?", reference=ctx.message)

        await vc.resume()
        await ctx.send("Lagu dilanjutkan kembali!", reference=ctx.message)


async def setup(bot: naoTimesBot):
    await bot.add_cog(MusikPlayerCommand(bot))
