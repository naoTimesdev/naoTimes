...
class PeninjauKursMataUang(commands.Cog):
    ...
    @app_commands.command(name="kurs")
    @app_commands.describe(
        dari="Mata uang awal", ke="Mata uang tujuan", jumlah="Jumlah mata uang yang ingin ditukar"
    )
    async def _peninjau_kurs_slash(
        self, inter: discord.Interaction, dari: str, ke: str, jumlah: Optional[str] = None
    ):
...
async def setup(bot: naoTimesBot):
    await bot.add_cog(PeninjauKursMataUang(bot))
