...
def parse_args(str_txt: str, s: str, search_mode=True):
    ...
    parser = BotArgumentParser(prog="!nyaa " + s, usage="!nyaa " + s, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    if search_mode:
        parser.add_argument('input', help='Apa yang mau dicari')
    parser.add_argument('--category', '-C', required=False, default="all", dest='kategori', action='store', help='Kategori pencarian - cek dengan !nyaa kategori')
    parser.add_argument('--user', '-u', required=False, default=None, dest='user', action='store', help='Cari torrent hanya pada user yang diberikan')
    parser.add_argument('--sukebei', '-nsfw', required=False, dest='sukebei', action='store_true', help='Mode Sukebei')
    parser.add_argument('--trusted', required=False, dest='trust_only', action='store_true', help='Filter untuk torrent trusted saja')
    parser.add_argument('--no-remake', '-nr', required=False, dest='no_remake', action='store_true', help='Filter untuk torrent yang bukan remake/merah')
    try:
...
async def fetch_nyaa(keyword=None, category='all', trusted=False, nr=False, user=None, sukebei=False):
    """
    ...
    if trusted and nr:
        return 'Pilih antara `--trusted` atau `--no-remake` (Hanya bisa salah satu)'
...
class NyaaTorrentsV2(commands.Cog):
    ...
    async def cari(self, ctx, *, args_='-h'):
        ...
        nqres = await fetch_nyaa(args.input, args.kategori, args.trust_only, args.no_remake, args.user, args.sukebei)
        ...
        while True:
            if first_run:
                ...
                embed.set_author(name=data['name'], url=data['url'], icon_url="https://nyaa.si/static/img/avatar/default.png")
                embed.set_footer(text='{} | Dibuat: {}'.format(data['id'], data['creation']))

                se, le, co = data['seeders'], data['leechers'], data['completed']
                dl_link_fmt = '📥 \|| **[Torrent]({t})**'.format(t=data['download_link'])

                embed.add_field(name="Uploader", value=data['submitter'], inline=True)
                embed.add_field(name="Kategori (ID)", value=data['category'] + ' ({})'.format(data['category_id']), inline=True)
                embed.add_field(name="Stats", value='**Seeders**: {}\n**Leechers**: {}\n**Completed**: {}'.format(se, le, co), inline=False)
    ...
    async def terbaru(self, ctx, *, args_='-h'):
        ...
        if isinstance(args, str):
            return await ctx.send(parse_error(args))
        nqres = await fetch_nyaa(None, args.kategori, args.trust_only, args.no_remake, args.user, args.sukebei)
        if isinstance(nqres, str):
            return await ctx.send(nqres)
