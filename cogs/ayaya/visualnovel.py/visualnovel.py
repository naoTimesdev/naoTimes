...
class AyayaVisualNovel(commands.Cog):
    ...
    def _design_embed(data: VNDBModel) -> discord.Embed:
        ...
        embed.add_field(name="Sinopsis", value=cutoff_text(data.description, 1024), inline=False)
        return embed
    ...
    async def _ayaya_vnmain(self, ctx: naoTimesContext, *, judul: str):
        ...
        async def _screenies_handler(data: VNDBModel, _, message: discord.Message, view: DiscordPaginatorUI):
            screen_gen = DiscordPaginatorUI(view.ctx, data.screenies)
            img_embed_gen = partial(self._design_screenies, data=data)
            screen_gen.attach(img_embed_gen)
            screen_gen.parent = view
            return screen_gen

        main_gen = DiscordPaginatorUI(ctx, vnqres)
        main_gen.attach(self._design_embed)
        main_gen.add_handler(
            "Screenshot", lambda data: len(data.screenies) > 0, _screenies_handler, emoji="📸"
        )
...
async def setup(bot: naoTimesBot):
    await bot.add_cog(AyayaVisualNovel(bot))
