...
class ModToolsAutomoderator(commands.Cog):
    ...
    async def _modtools_automod_add(self, ctx: naoTimesContext):
        ...
        selections = [
            Selection("Tambah kata baru", "add", "Tambah kata baru ke automod", emoji="➕"),
            Selection("Tambah regex", "add_regex", "Tambah regex baru ke automod", emoji="🔨"),
            Selection("Selesai", "done", "Selesai edit, tambahkan beneran ke database", emoji="✅"),
        ]
        ...
        while True:
            ...
            selected = await ctx.select_single_button(target_msg, selections, 60.0)
            if selected is None:
                msg_cancelled = True
                msg_timeout = True
                break
            ...
            elif selected == "add_regex":
                await ask_regex()
            elif selected == "done":
                break

        if msg_cancelled and msg_timeout:
    ...
    @mdt_automod.command(name="hapus", aliases=["delete"])
    async def _modtools_automod_delete(self, ctx: naoTimesContext):
        guild_id = ctx.guild.id

        if guild_id not in self._manager:
            return await ctx.send("👮⚙️ Automod belum diaktifkan!")

        reflex = "**Berikut adalah list kata yang bisa anda hapus**\n"
        all_words = self._manager.get_child(guild_id).words
        for nn, word in enumerate(all_words, 1):
            if isinstance(word, str):
                reflex += f"**{nn}**. `{word}`\n"
            else:
                reflex += f"**{nn}**. `{word[:20]} [...]` (Regex)\n"
        await ctx.send(reflex)

        delete_this = False
        while True:
            delete_this = await ctx.wait_content(
                "Mohon ketik nomor kata yang ingin anda hapus", True, True, timeout=None
            )
            if delete_this is False:
                break
            try:
                delete_this = int(delete_this)
                if delete_this < 1 or delete_this > len(all_words):
                    await ctx.send_timed("Nomor yang dimasukan tidak valid, mohon ulangi!", 2)
                else:
                    break
            except ValueError:
                await ctx.send_timed("Nomor yang anda masukkan bukanlah angka!", 2)

        if not isinstance(delete_this, int):
            return await ctx.send("*Dibatalkan*")

        delete_this -= 1
        self._manager.get_child(guild_id).remove_at(delete_this)
        await self.bot.redisdb.set(
            f"ntmodtools_yabai_{guild_id}", self._manager.get_child(guild_id).serialize()
        )
        await ctx.send("👮⚙️ Kata berhasil dihapus!")
...
async def setup(bot: naoTimesBot):
    await bot.add_cog(ModToolsAutomoderator(bot))
