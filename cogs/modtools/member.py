...
class MemberManager:
    def __init__(self, Context: MemberManagerContext):
        self._childs: List[MemberManagerContext] = []
        self._ctx = Context
    ...
    def add_child(self, guild_id: Any) -> MemberManagerContext:
        child = self._ctx(guild_id)
        self._childs.append(child)
        return child

    def get_child(self, guild_id: Any) -> MemberManagerContext:
        for child in self._childs:
            if child == guild_id:
                return child
    ...
    def get_from_child(self, guild_id, data: Any) -> Optional[Any]:
        child = self.get_child(guild_id)
        if data in child:
...
class ModToolsMemberControl(commands.Cog):
    def __init__(self, bot: naoTimesBot) -> None:
        ...
        self._shadowbanned = MemberManager(ShadowbanManagerChild)
        self._mute_manager = MemberManager(MuteManagerChild)
...
def setup(bot: naoTimesBot):
    bot.add_cog(ModToolsMemberControl(bot))
