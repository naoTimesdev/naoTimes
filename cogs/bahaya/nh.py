...
class NSFWCogNH(commands.Cog):
    ...
    async def wrap_start_image(
        ...
        reader_gen = DiscordPaginator(self.bot, ctx, dataset["images"])
        reader_gen.remove_at_trashed = break_apart
        reader_gen.set_generator(img_embed_gen)
        await reader_gen.paginate(None, message)
        return None, message
    ...
    async def wrap_start_dl(
        ...
        dl_gen.paginateable = False
        dl_gen.set_generator(self._fmt_download_embed)
        await dl_gen.paginate(None, message)
        return None, message
    ...
    async def wrap_start_info(
        ...
        info_gen.add_handler("📥", lambda x: True, dl_wrap_ctx)
        info_gen.add_handler("📖", lambda x: True, img_wrap_ctx)
        info_gen.set_generator(self._fmt_info_embed)
        await info_gen.paginate(None, message)
        return None, message
    ...
    async def _nh_nsfw_cari(self, ctx: naoTimesContext, *, query: str = None):
        ...
        main_gen = DiscordPaginator(self.bot, ctx, results)
        info_wrap_ctx = partial(self.wrap_start_info, ctx=ctx)
        main_gen.add_handler("📜", lambda x: True, info_wrap_ctx)
        search_gen = partial(self._fmt_search_embed, query=query)
        main_gen.set_generator(search_gen)
        await main_gen.paginate()
