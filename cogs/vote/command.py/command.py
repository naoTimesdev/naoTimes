...
class VoteAppCommand(commands.Cog):
    ...
    @commands.guild_only()
    async def _vote_main(
        self, ctx: naoTimesContext, *, raw_args: vote_converter = vote_converter.show_help()
    ):
        if isinstance(raw_args, str):
            return await ctx.send(quoteblock(raw_args, code="py"))

        args = cast(CommandArgParse, raw_args)
...
async def setup(bot: naoTimesBot):
    await bot.add_cog(VoteAppCommand(bot))
