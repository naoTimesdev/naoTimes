...
class ShowtimesAlias(commands.Cog):
    ...
    async def _showalias_main(self, ctx: naoTimesContext):
        if not ctx.invoked_subcommand:
            ...
            async def _select_project():
                ...
                find_match = srv_data.find_projects(await_this, True)
                if len(find_match) < 1:
                ...
                if len(find_match) > 1:
                    ...
                    selected_project = await ctx.select_simple(find_match, lambda x: x.title)
    ...
    async def _showalias_list(self, ctx: naoTimesContext, *, judul: str = None):
        ...
        all_matches = srv_data.find_projects(judul, True)
...
def setup(bot: naoTimesBot):
    if bot.ntdb is None:
        ...
        return
    bot.add_cog(ShowtimesAlias(bot))
