...
class ShowtimesStaff(commands.Cog):
    ...
    async def _do_release(
        ...
        all_matches = await self.bot.loop.run_in_executor(None, server.find_projects, title)
    ...
    async def _do_beres_batal(
        ...
        all_matches = await self.bot.loop.run_in_executor(None, server.find_projects, judul)
        if len(all_matches) < 1:
...
async def setup(bot: naoTimesBot):
    if bot.ntdb is None:
        ...
        return
    await bot.add_cog(ShowtimesStaff(bot))
