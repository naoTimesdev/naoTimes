...
class ShowtimesOwner(commands.Cog):
    ...
    @_showowner_main.command(name="debuglist")
    async def _showowner_debuglist(self, ctx: naoTimesContext):
        self.logger.info("Force pushing local data to main database")
        srv_id = ctx.guild.id
        get_sh_server = await self.bot.showqueue.fetch_database(srv_id)
        if not get_sh_server:
            return await ctx.send("Server not found!")

        res = await ctx.select_single(
            get_sh_server.projects,
...
async def setup(bot: naoTimesBot):
    await bot.add_cog(ShowtimesOwner(bot))
