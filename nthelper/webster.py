...
def str_or_none(text: T.Union[str, None], default: str = ""):
    if text is None:
        return default
    return text


def list_or_none(lists: T.Union[list, None], default: list = []):
    if not isinstance(lists, list):
        return default
    return lists


def get_pos(data: list, n: int):
    try:
        return data[n]
...
def complex_walk(dictionary: T.Union[dict, list], paths: str):
    ...
    for n, path in enumerate(expanded_paths):
        ...
        if path == "*" and isinstance(dictionary, list):
            new_concat = []
            next_path = get_pos(expanded_paths, n + 1)
    ...
    def _parse_thesaurus_results(self, api_results: T.List[dict], word_fallback: str):
        ...
        for words in api_results:
            ...
            for n, defs in enumerate(shortdef):
                defant = list_or_none(get_pos(antonyms, n))
                defsyn = get_pos(synonyms, n)
