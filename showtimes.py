...
class Showtimes:
    ...
    async def globalpatcher(self, ctx):
        ...
        for srv in srv_list:
            ...
            for k in json_d[srv]['anime']:
                print('@@ Processing anime: ' + k)
                _, poster_image, title, time_data, correct_episode_num = await fetch_anilist(json_d[srv]['anime'][k]['anilist_id'], 1, list(json_d[srv]['anime'][k]['status'].keys())[-1], True)
                for x in range(correct_episode_num):
                    try:
                        json_d[srv]['anime'][k]['status'][str(x+1)]['airing_time'] = time_data[x]
                    except IndexError:
                        pass
                    except KeyError:
                        pass
        ...
        preview_msg = await self.bot.say('**This will patch and modify and add airing_time**\nDo you want to continue?')
        to_react = ['✅', '❌']
        for reaction in to_react:
